...
export function POSCart({
  items, onUpdateQuantity, onRemoveItem, onClearCart,
  discount, discountType, onDiscountChange, onDiscountTypeChange, onFinalize,
}: POSCartProps) {
  const subtotal = items.reduce((sum, item) => sum + item.product.sell_price * item.quantity, 0);
            ...
            {items.map((item) => (
              <div key={item.product.id} className="flex items-center gap-2 rounded-md border p-2 bg-background">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{formatKz(item.product.sell_price)}/un</p>
                </div>
                ...
                <span className="text-sm font-semibold w-20 text-right">{formatKz(item.product.sell_price * item.quantity)}</span>
      ...
      <div className="border-t p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 flex-1">
            <Input type="number" min={0} className="h-8 text-sm" value={discount || ""} onChange={(e) => onDiscountChange(Number(e.target.value))} placeholder="0" />
            <Button variant={discountType === "percent" ? "default" : "outline"} size="icon" className="h-8 w-8 shrink-0" onClick={() => onDiscountTypeChange("percent")}>
              <Percent className="h-3 w-3" />
            </Button>
            <Button variant={discountType === "fixed" ? "default" : "outline"} size="icon" className="h-8 w-8 shrink-0" onClick={() => onDiscountTypeChange("fixed")}>
