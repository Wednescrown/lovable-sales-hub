...
const StockAdjustmentPage = () => {
  ...
  const selectedProduct = mockProducts.find((p) => p.id === formProductId);
  ...
  // KPIs
  const todayAdjustments = adjustments.filter((a) => a.date.startsWith("2026-02-16"));
  ...
  const handleSubmit = () => {
    ...
    const store = mockStores.find((s) => s.id === formStore);
    const counted = parseInt(formCountedQty) || 0;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Registre ajustes manuais de estoque por loja
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Ajustes Hoje</p>
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Ajustes Positivos</p>
                <p className="text-lg font-bold text-success">+{positiveToday}</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Ajustes Negativos</p>
                <p className="text-lg font-bold text-destructive">{negativeToday}</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold">
              Histórico de Ajustes ({filteredAdjustments.length})
            </CardTitle>
          </CardHeader>
              ...
              <TableBody>
                {filteredAdjustments.map((adj) => (
                  <TableRow key={adj.id}>
                    ...
                    <TableCell className="text-center">
                      <span className={`text-sm font-bold ${adj.difference > 0 ? "text-success" : "text-destructive"}`}>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Selecione a filial e o produto para registrar o ajuste.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="grid gap-4 py-2">
              ...
              <div className="grid gap-1.5">
                ...
                <Input
                  ...
                  value={formSearch}
                  onChange={(e) => { setFormSearch(e.target.value); setFormProductId(""); }}
                />
                {filteredProducts.length > 0 && !formProductId && (
                  <div className="border rounded-md bg-popover shadow-sm max-h-40 overflow-y-auto">
                    {filteredProducts.map((p) => (
                      <button
                        key={p.id}
                        className="w-full text-left px-3 py-2 hover:bg-accent text-sm flex items-center gap-2 border-b last:border-b-0"
                        onClick={() => { setFormProductId(p.id); setFormSearch(p.name); }}
                      >
                        <Package className="w-3.5 h-3.5 text-muted-foreground shrink-0" />
                        <div>
                          <p className="text-sm font-medium">{p.name}</p>
                          <p className="text-[10px] text-muted-foreground">{p.barcode} · {p.sku}</p>
                        </div>
                      </button>
                    ))}
                  </div>
                  ...
                  <CardContent className="p-3">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-muted-foreground">{selectedProduct.categoryName} › {selectedProduct.subcategoryName}</p>
                      </div>
              ...
              <div className="grid gap-1.5">
                ...
                <div className="flex gap-2">
                  <Button
                    type="button"
                    variant={formType === "add" ? "default" : "outline"}
                    size="sm"
                    className={formType === "add" ? "bg-success hover:bg-success/90 text-success-foreground" : ""}
                  ...
                  <Button
                    type="button"
                    variant={formType === "reduce" ? "default" : "outline"}
                    size="sm"
                    className={formType === "reduce" ? "bg-destructive hover:bg-destructive/90 text-destructive-foreground" : ""}
                    onClick={() => setFormType("reduce")}
                  >
              ...
              <div className="grid gap-1.5">
                ...
                <Input
                  ...
                  placeholder="0"
                  className="h-9 text-sm"
                  value={formCountedQty}
                  onChange={(e) => setFormCountedQty(e.target.value)}
                />
              </div>
              ...
              {selectedProduct && formCountedQty && (
                <div className={`p-3 rounded-lg border ${difference > 0 ? "bg-success/10 border-success/30" : "bg-destructive/10 border-destructive/30"}`}>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Novo stock: <span className="font-medium text-foreground">{selectedProduct.stock + difference}</span>
                  </p>
                </div>
              )}
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={!selectedProduct || !formCountedQty || !formReason || !formStore}
              >
                Confirmar Ajuste
              </Button>
            </DialogFooter>
