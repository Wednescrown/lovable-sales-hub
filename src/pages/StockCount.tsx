...
import {
  ClipboardCheck, Search, Save, CheckCircle2, Package, Barcode, Edit, AlertTriangle,
  TrendingDown, TrendingUp, CalendarIcon,
} from "lucide-react";
...
import { mockStores, formatKz, type CountItem } from "@/data/mockInventory";
...
const StockCountPage = () => {
  ...
  const addProduct = (productId: string, countedQty: number = 0) => {
    ...
    if (existing) {
      ...
      setCountItems([
        ...countItems,
        {
          productId: product.id, productName: product.name, systemQty: product.stock, countedQty,
          differenceQty: diff, costPrice: product.costPrice, sellPrice: product.sellPrice,
          differenceCostValue: diff * product.costPrice, differenceSellValue: diff * product.sellPrice,
        },
      ]);
    }
    setProductSearch("");
  ...
  const updateCountedQty = (productId: string, qty: number) => {
    ...
    setCountItems(
      countItems.map((c) =>
        c.productId === productId
          ? { ...c, countedQty: qty, differenceQty: qty - c.systemQty, differenceCostValue: (qty - c.systemQty) * product.costPrice, differenceSellValue: (qty - c.systemQty) * product.sellPrice }
          : c
      )
    );
    setEditingId(null);
  ...
  const handleFinalize = () => {
    if (uncountedProducts.length > 0) setUncountedOpen(true);
    else setPeriodOpen(true);
  };
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Realize a contagem completa do inventário por filial</p>
          </div>
          <div className="flex items-center gap-2">
            <Select value={selectedStore} onValueChange={setSelectedStore}>
              <SelectTrigger className="w-[180px] h-9 text-sm"><SelectValue placeholder="Selecionar Filial" /></SelectTrigger>
              <SelectContent>
            ...
            <Button size="sm" onClick={handleFinalize} disabled={countItems.length === 0}>
              <CheckCircle2 className="w-4 h-4 mr-1" />Finalizar Inventário
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div><p className="text-xs text-muted-foreground">Perda em Stock (Custo)</p><p className="text-lg font-bold text-destructive">{formatKz(Math.abs(totalCostLoss))}</p></div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div><p className="text-xs text-muted-foreground">Diferença Valor Venda</p><p className={`text-lg font-bold ${diffColor(totalSellDiff)}`}>{diffPrefix(totalSellDiff)}{formatKz(totalSellDiff)}</p></div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div><p className="text-xs text-muted-foreground">Balanço Geral</p><p className={`text-lg font-bold ${diffColor(generalBalance)}`}>{diffPrefix(generalBalance)}{formatKz(generalBalance)}</p></div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div><p className="text-xs text-muted-foreground">Produtos Contados</p><p className="text-lg font-bold text-foreground">{countedCount} <span className="text-xs font-normal text-muted-foreground">/ {totalStoreProducts}</span></p></div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="relative">
              ...
              <Input placeholder="Pesquisar produto por nome ou código de barras para adicionar..." className="pl-9 h-9 text-sm" value={productSearch} onChange={(e) => setProductSearch(e.target.value)} />
            </div>
                ...
                {searchResults.map((p) => (
                  <button key={p.id} className="w-full text-left px-3 py-2 hover:bg-accent text-sm flex items-center justify-between border-b last:border-b-0" onClick={() => openProductDetail(p)}>
                    <div className="flex items-center gap-2">
                      ...
                      <div><p className="text-sm font-medium">{p.name}</p><p className="text-[10px] text-muted-foreground">{p.barcode} · Stock: {p.stock}</p></div>
                    </div>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {countItems.length > 0 && (
                <div className="relative w-64">
                  ...
                  <Input placeholder="Filtrar na lista..." className="pl-8 h-8 text-xs" value={listSearch} onChange={(e) => setListSearch(e.target.value)} />
                </div>
              )}
            </div>
          </CardHeader>
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                  ...
                  <TableRow key={item.productId}>
                    ...
                    <TableCell className="text-center">
                      {editingId === item.productId ? (
                        <Input type="number" className="h-7 w-20 text-sm text-center mx-auto" value={editValue} autoFocus onChange={(e) => setEditValue(e.target.value)} onBlur={() => updateCountedQty(item.productId, parseInt(editValue) || 0)} onKeyDown={(e) => { if (e.key === "Enter") updateCountedQty(item.productId, parseInt(editValue) || 0); }} />
                      ) : (
                    ...
                    <TableCell className="text-center">
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => { setEditingId(item.productId); setEditValue(item.countedQty.toString()); }}>
                ...
                {countItems.length > 0 && filteredCountItems.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-8 text-sm text-muted-foreground">Nenhum produto encontrado na lista.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
        ...
        <Dialog open={!!detailProduct} onOpenChange={(open) => { if (!open) setDetailProduct(null); }}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>Insira a quantidade contada para adicionar à lista.</DialogDescription>
            </DialogHeader>
            {detailProduct && (
              <div className="space-y-4 py-2">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div><p className="text-xs text-muted-foreground">Categoria</p><p>{detailProduct.categoryName}</p></div>
                  <div><p className="text-xs text-muted-foreground">Stock no Sistema</p><p className="font-bold text-lg">{detailProduct.stock}</p></div>
                  <div><p className="text-xs text-muted-foreground">Preço Custo</p><p>{formatKz(detailProduct.costPrice)}</p></div>
                  <div><p className="text-xs text-muted-foreground">Preço Venda</p><p className="font-medium">{formatKz(detailProduct.sellPrice)}</p></div>
                </div>
                <div className="grid gap-1.5">
                  ...
                  <Input
                    ...
                    onKeyDown={(e) => { if (e.key === "Enter") confirmProductDetail(); }}
                  />
                </div>
              </div>
            )}
            <DialogFooter>
              <Button variant="outline" onClick={() => setDetailProduct(null)}>Cancelar</Button>
              <Button onClick={confirmProductDetail}>Adicionar à Lista</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={uncountedOpen} onOpenChange={setUncountedOpen}>
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-warning" />Produtos Não Contados</DialogTitle>
              <DialogDescription>Os seguintes produtos possuem stock no sistema mas não foram incluídos na contagem.</DialogDescription>
            </DialogHeader>
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {uncountedProducts.map((p) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="text-center">
                      <Input type="number" min="0" placeholder="0" className="h-7 w-20 text-sm text-center mx-auto" value={uncountedQtys[p.id] || ""} onChange={(e) => setUncountedQtys({ ...uncountedQtys, [p.id]: e.target.value })} />
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => { setUncountedOpen(false); setPeriodOpen(true); }}>Ignorar e Continuar</Button>
              <Button onClick={handleUncountedContinue}>Adicionar à Contagem</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={periodOpen} onOpenChange={setPeriodOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2"><ClipboardCheck className="w-5 h-5 text-primary" />Resumo do Inventário</DialogTitle>
              <DialogDescription>Defina o período e revise o resumo final.</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-2">
              <div className="grid gap-1.5">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className={cn("w-full justify-start text-left font-normal h-9 text-sm", !periodStartDate && "text-muted-foreground")}>
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {periodStartDate ? format(periodStartDate, "dd/MM/yyyy", { locale: pt }) : "Selecionar data"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar mode="single" selected={periodStartDate} onSelect={setPeriodStartDate} initialFocus className={cn("p-3 pointer-events-auto")} />
                  </PopoverContent>
                </Popover>
              </div>
              <div className="grid gap-3">
                ...
                <div className={`flex items-center justify-between p-3 rounded-lg border ${generalBalance >= 0 ? "bg-success/10 border-success/20" : "bg-destructive/10 border-destructive/20"}`}>
                  ...
                  <span className={`text-sm font-bold ${diffColor(generalBalance)}`}>{diffPrefix(generalBalance)}{formatKz(generalBalance)}</span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50 border">
